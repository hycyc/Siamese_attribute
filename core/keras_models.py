...
def model_Siamese(p, embedding_matrix, max_sent_len, n_out):
    ...
    Ew = layers.Lambda(function=cosine_distance, output_shape=eucl_dist_output_shape)([processed_a, processed_b])

    model = models.Model(inputs=[sentence_input1, entity_markers1, sentence_input2, entity_markers2], outputs=Ew)

    #rmsprop = RMSprop()
    model.compile(optimizer=p['optimizer'], loss=contrastive_loss, metrics=[accuracy])
